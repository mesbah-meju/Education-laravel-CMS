...
@section('contents')
        ...
        <div class="card border-0 shadow-sm rounded">
            <div class="card-header bg-white border-0 d-flex justify-content-between align-items-center py-3">
                <div class="d-flex align-items-center gap-2">
                    ...
                    <h6 class="mb-0 fw-bold">Exam Routine File Upload</h6>
                </div>
                <button class="btn btn-soft-primary btn-sm rounded-pill" data-bs-toggle="modal"
                    data-bs-target="#addExamRoutineModal">
            ...
            <div class="card-body">
                ...
                <div class="row g-2 mb-3 align-items-center">
                    ...
                    <div class="col-md-4 col-sm-12">
                        <input type="text" id="examroutineSearch" class="form-control form-control-sm"
                            placeholder="🔍 Search...">
                    </div>
                        ...
                        <tbody class="text-muted">
                            @foreach($examroutines as $examroutine)
                                <tr>
                                    ...
                                    <td>{{ $examroutine->class->name }}</td>
                                    <td>{{ $examroutine->shift }}</td>
                                    <td>{{ $examroutine->routine_title }}</td>
                                    <td>{{ $examroutine->published_date }}</td>
                                    <td>
                                        @if($examroutine->file_path)
                                            ...
                                            <a href="{{ route('examroutine.download', $examroutine->id) }}"
                                    ...
                                    <td class="text-end">
                                        ...
                                        <a href="{{ route('examroutine.status', $examroutine->id) }}"
                                            class="btn @if($examroutine->is_active == 1) btn-soft-success @else btn-soft-danger @endif btn-sm rounded-2">
                                            <i class="fas fa-check"></i>
                                        </a>
                                        <button type="button" class="btn btn-soft-primary btn-sm rounded-2 btn-edit-file"
                                            data-bs-toggle="modal" data-bs-target="#editExamRoutineModal"
                                            data-id="{{ $examroutine->id }}" data-class_id="{{ $examroutine->class_id }}"
                                            data-shift="{{ $examroutine->shift }}"
                                            data-routine_title="{{ $examroutine->routine_title }}"
                                            data-published_date="{{ $examroutine->published_date }}"
                                            data-file="{{ $examroutine->file_path }}"
                                            data-action="{{ route('examroutine.update', ':id') }}">
                                            <i class="fas fa-edit"></i>
                                        </button>

                                        <form action="{{ route('examroutine.destroy', $examroutine->id) }}" method="POST"
                                            class="d-inline"
    ...
    <!-- Add Modal -->
    <div class="modal fade" id="addExamRoutineModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content rounded border-0 shadow-sm">
                <form action="{{ route('examroutine.store') }}" method="POST" enctype="multipart/form-data"
    ...
    <!-- Edit Modal -->
    <div class="modal fade" id="editExamRoutineModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content rounded border-0 shadow-sm">
                <form id="editExamRoutineForm" method="POST" enctype="multipart/form-data" class="needs-validation"
                    novalidate>
                    @csrf
                    @method('PUT')
                    <div class="modal-header border-0">
                    ...
                    <div class="modal-body text-muted">
                        <input type="hidden" name="examroutine_id" id="examroutine_id">
                        <div class="row">
                            <div class="mb-3">
                                <label class="form-label">Exam Title</label>
...
@section('scripts')
    <script>
        ...
        // Edit modal populate
        $('#editExamRoutineModal').on('show.bs.modal', function (event) {
            ...
            var form = $('#editExamRoutineForm');
            form.attr('action', action.replace(':id', id));
            form.find('#examroutine_id').val(id);
        ...
        // Text + Date filter
        function Filters() {
            const query = $('#examroutineSearch').val().toLowerCase();
        ...
        // Trigger search
        $('#examroutineSearch').on('keyup', Filters);
