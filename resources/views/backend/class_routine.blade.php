...
@section('contents')
        ...
        <div class="card border-0 shadow-sm rounded">
            <div class="card-header bg-white border-0 d-flex justify-content-between align-items-center py-3">
                <div class="d-flex align-items-center gap-2">
                    ...
                    <h6 class="mb-0 fw-bold">Routine File Upload</h6>
                </div>
                <button class="btn btn-soft-primary btn-sm rounded-pill" data-bs-toggle="modal"
                    data-bs-target="#addClassRoutineModal">
            ...
            <div class="card-body">
                ...
                <div class="row g-2 mb-3 align-items-center">
                    ...
                    <div class="col-md-4 col-sm-12">
                        <input type="text" id="classroutineSearch" class="form-control form-control-sm"
                            placeholder="🔍 Search...">
                    </div>
                    ...
                    <table id="table-data" class="table mb-0 align-middle">
                        <thead class="table-light">
                            <tr class="text-uppercase text-muted fw-semibold" style="letter-spacing: 0.05em;">
                                ...
                                <th class="py-2">Routine Title</th>
                        ...
                        <tbody class="text-muted">
                            @foreach($classroutines as $classroutine)
                                <tr>
                                    ...
                                    <td>{{ $classroutine->class->name }}</td>
                                    <td>{{ $classroutine->shift }}</td>
                                    <td>{{ $classroutine->routine_title }}</td>
                                    <td>{{ $classroutine->published_date }}</td>
                                    <td>
                                        @if($classroutine->file_path)
                                            ...
                                            <a href="{{ route('classroutine.download', $classroutine->id) }}"
                                    ...
                                    <td class="text-end">
                                        ...
                                        <a href="{{ route('classroutine.status', $classroutine->id) }}"
                                            class="btn @if($classroutine->is_active == 1) btn-soft-success @else btn-soft-danger @endif btn-sm rounded-2">
                                            <i class="fas fa-check"></i>
                                        </a>
                                        <button type="button" class="btn btn-soft-primary btn-sm rounded-2 btn-edit-file"
                                            data-bs-toggle="modal" data-bs-target="#editClassRoutineModal"
                                            data-id="{{ $classroutine->id }}" data-class_id="{{ $classroutine->class_id }}"
                                            data-shift="{{ $classroutine->shift }}"
                                            data-routine_title="{{ $classroutine->routine_title }}"
                                            data-published_date="{{ $classroutine->published_date }}"
                                            data-file="{{ $classroutine->file_path }}"
                                            data-action="{{ route('classroutine.update', ':id') }}">
                                            <i class="fas fa-edit"></i>
                                        </button>

                                        <form action="{{ route('classroutine.destroy', $classroutine->id) }}" method="POST"
                                            class="d-inline"
    ...
    <!-- Add Modal -->
    <div class="modal fade" id="addClassRoutineModal" tabindex="-1" aria-hidden="true">
                    ...
                    <div class="modal-body text-muted">
                        <div class="row">
                            <div class="mb-3">
                                ...
                                <input type="text" name="routine_title" class="form-control" required>
                                <div class="invalid-feedback">Routine title is required.</div>
                            </div>
    ...
    <!-- Edit Modal -->
    <div class="modal fade" id="editClassRoutineModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content rounded border-0 shadow-sm">
                <form id="editClassRoutineForm" method="POST" enctype="multipart/form-data" class="needs-validation"
                    novalidate>
                    @csrf
                    @method('PUT')
                    <div class="modal-header border-0">
                    ...
                    <div class="modal-body text-muted">
                        <input type="hidden" name="classroutine_id" id="classroutine_id">
                        <div class="row">
                            <div class="mb-3">
                                <label class="form-label">Routine Title</label>
                                <input type="text" name="routine_title" id="routine_title" class="form-control" required>
                                <div class="invalid-feedback">Routine title is required.</div>
                            </div>
...
@section('scripts')
    <script>
        ...
        // Edit modal populate
        $('#editClassRoutineModal').on('show.bs.modal', function (event) {
            var button = $(event.relatedTarget);
            var id = button.data('id');
            var class_id = button.data('class_id');
            var shift = button.data('shift');
            var routine_title = button.data('routine_title');
            var published_date = button.data('published_date');
            var is_active = button.data('is_active');
            var action = button.data('action');

            var form = $('#editClassRoutineForm');
            form.attr('action', action.replace(':id', id));
            form.find('#classroutine_id').val(id);
            form.find('#class_id').val(class_id);
            form.find('#shift').val(shift);
            form.find('#routine_title').val(routine_title);
            form.find('#published_date').val(published_date);
            form.find('#is_active_edit').prop('checked', is_active == 1);
        });

        // Text + Date filter
        function Filters() {
            const query = $('#classroutineSearch').val().toLowerCase();
        ...
        // Trigger search
        $('#classroutineSearch').on('keyup', Filters);
